...
export function SystemScroll() {
  ...
  useEffect(() => {
    ...
    const applyFit = () => {
      if (!heading || !shell || !timeline) return;
      section.classList.remove("system-scroll--tight");
      section.style.removeProperty("--system-scale");
      ...
      const gap = 16;
      const available = timelineTop - minTop - gap;
      let shellHeight = shell.offsetHeight;

      if (available > 0 && shellHeight > available) {
        section.classList.add("system-scroll--tight");
        shellHeight = shell.offsetHeight;
      }

      let scale = 1;
      if (available > 80 && shellHeight > available) {
        scale = Math.max(0.72, available / shellHeight);
        section.style.setProperty("--system-scale", scale.toFixed(3));
      }

      const visualHeight = shellHeight * scale;
      const maxTop = timelineTop - visualHeight - gap;
      const top = maxTop < minTop ? Math.max(8, maxTop) : Math.min(Math.max(preferred, minTop), maxTop);
